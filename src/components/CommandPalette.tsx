...
function CommandPaletteDialog({ open, onOpenChange }: DialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="sm:max-w-[640px] p-0 gap-0 overflow-hidden border-white/[0.08] bg-[#0a0a0d]/98 backdrop-blur-2xl shadow-[0_24px_80px_rgba(0,0,0,0.7)] [&>button:last-child]:hidden"
      >
        <VisuallyHidden>
        ...
        >
          ...
          <div className="flex items-center gap-3 px-4 h-14 border-b border-white/[0.06]">
            ...
            <CommandPrimitive.Input
              ...
              autoFocus
              placeholder="Search 97 tools — what do you want to do?"
              className="flex-1 bg-transparent border-0 outline-none text-[15px] text-white placeholder:text-white/30"
            />
            {query && (
              <button
                onClick={() => setQuery('')}
            ...
            <kbd className="hidden sm:inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-white/[0.05] border border-white/[0.08] text-[10px] font-semibold text-white/40">
              ESC
            </kbd>
          </div>
          ...
          {/* Footer */}
          <div className="flex items-center justify-between px-4 py-2.5 border-t border-white/[0.06] bg-black/20 text-[11px] text-white/35">
            ...
            <span className="hidden sm:inline">{TOOLS.length} tools across {Object.keys(CATEGORIES).length} categories</span>
          </div>
        </CommandPrimitive>
      </DialogContent>
    </Dialog>
...
function Group({ label, icon, dot, children }: GroupProps) {
  ...
  return (
    <CommandPrimitive.Group
      heading={heading}
      className="mb-1 [&_[cmdk-group-heading]]:px-2.5 [&_[cmdk-group-heading]]:py-2"
    >
...
function ToolItem({ tool, onSelect }: ToolItemProps) {
  ...
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        'group flex items-center gap-3 px-2.5 py-2 rounded-xl cursor-pointer',
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="text-[13px] font-medium text-white/85 truncate">{tool.title}</span>
          {tool.badge && (
            <span className={cn(
      ...
      <ArrowRight className="h-3.5 w-3.5 text-white/20 group-data-[selected=true]:text-white/55 transition-colors shrink-0" />
    </CommandPrimitive.Item>
  );
}
